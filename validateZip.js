...
import zipRangesEnd from './zipRangesEnd';
import zipRangesStart from './zipRangesStart';
import invalidZip from './invalidZip';

const findZipRange = (input, ranges) => {
  // false means invalid
  let pivot = 33;
  let start = 0;
  let end = 66;
  ...
  const section = (pivot, position) => {
    let index = 0;
    if (position === 1) {
      if (typeof ranges[pivot - 1]) {
        index = ranges[pivot].length - 1;
      }
    }
    return ranges[pivot][index][position];
  };
  // 34470
  for (let i = 0; i < 4; i++) {
    do {
      ...
      if (input < section(pivot, 1)) {
        ...
        /*         if (typeof ranges[pivot + 1] !== 'undefined') {
...
const validateZip = (input) => {
  ...
  const invalidRanges = zipRanges(zipRangesStart, zipRangesEnd);
  // console.log(invalidRanges[49])
  const sectionId = findZipRange(input, invalidRanges);
  // console.log(sectionId);
  if (sectionId || sectionId === 0) {
    const section = invalidRanges[sectionId];
